...
gm_12878_regions = '/nfs/proj/EPInformer-AS/AS-Epinformer-Dup/data/GM12878_DNase_ENCFF020WZB_hic_4DNFI1UEG1HD_1MB_ABC_nominated/DNase_ENCFF020WZB_Neighborhoods/GeneList.TSS1kb.bed'
k562_regions = '/nfs/proj/EPInformer-AS/AS-Epinformer-Dup/data/K562_DNase_ENCFF257HEE_hic_4DNFITUOMFUQ_1MB_ABC_nominated/DNase_ENCFF257HEE_Neighborhoods/GeneList.TSS1kb.bed'
...
def process_histone_signals():
    ...
    for histone_type, tissues in file_uuids.items():
        for tissue, uuid in tissues.items():
            ...
            output_file = f"{output_dir}/Promoters{tissue}.{histone_type}.tab"
...
def combine_signal_files(output_files, column='mean'):
    ...
    for tissue, dfs in output_dfs.items():
        ...
        combined_df.to_csv(f"{output_dir}/Promoter_Combined_{tissue}_Histone_Signals.csv", index=False)
        print(f"Combined data for {tissue} saved to {output_dir}/Promoter_Combined_{tissue}_Histone_Signals.csv")
