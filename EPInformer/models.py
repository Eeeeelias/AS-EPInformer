...
class EPInformer_v2(nn.Module):
    def __init__(self, base_size = 4, n_encoder=3, out_dim=128, head = 4, pre_trained_encoder= None, n_enhancer=50, 
                 ...
                 exon_data=False, separate_attention=False, name_add=None):
        ...
        attn_mask = (~np.identity(self.n_enhancer+1).astype(bool)) # changed +1 to +4 to account for the exon segments
        attn_mask[:, 0] = False
        attn_mask[0, :] = False
